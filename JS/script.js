...
function checkUserSession() {
    ...
    if (user && authLink) {
        ...
        // If it's a Logout button (e.g. on admin dashboard), don't rename it, just bind the logout action
        if (authLink.textContent.includes('Logout')) {
        ...
        // Add Logout functionality to the button
        authLink.addEventListener('click', (e) => {
            e.preventDefault();
            localStorage.removeItem('currentUser');
            localStorage.removeItem('isAdmin');
        ...
        // Dynamic Dashboard Injection for Admin
        if (isAdmin) {
            const navUl = document.querySelector('nav ul');
            if (navUl && !navUl.querySelector('a[href="admin.html"]')) {
                const li = document.createElement('li');
                li.innerHTML = `<a href="admin.html" style="color: var(--primary-color); font-weight: bold;"><i class="fa-solid fa-gauge"></i> Dashboard</a>`;
                // Insert it before the user profile button (which is the last item)
...
function setActiveNavLink() {
    ...
    navLinks.forEach(link => {
        ...
        if (href === currentPage || pageFile === currentPage || 
